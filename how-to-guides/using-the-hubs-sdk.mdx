...
Once a user has authenticated to the Integration, the onConnected callback will provide connection information that you can use when sending a request to your Backend to authenticate the user in your system before then sending a POST request to the Versori create user endpoint.
...
app.post('/backendCreateUserEndpoint', () => {
    const body = {
        environments: [
            {
...
app.post('/backendDeleteUserEndpoint', () => {
    const reponse = fetch(`https://platform.versori.com/apis/switchboard/organisations/{orgId}/hubs/{hubId}/boards/{boardId}/users/{userId}`, {
        method: 'DELETE',
        headers: {
            Authorization: `Bearer [YOUR_WRITE_API_KEY]`,
