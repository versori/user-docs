...
To get started with the Versori Embedded SDK for React, you need to install the package. You can do this using npm or
yarn:

<CodeGroup>```bash npm npm install @versori/embed-react ``` ```bash yarn yarn add @versori/embed-react ```</CodeGroup>

# Usage

Once you have installed the package, you can import the `Embed` component from `@versori/embed-react` and use it in your
React application.
...
The `VersoriEmbeddedProvider` is a React component that initializes and provides the Versori Embedded Platform client to
its children via context. It also allows customization of some default settings and validation messages. Currently the
only default setting is the default integration tile image.
...
- **Required Properties**:
    ...
    - `endUserAuth`: Authentication details for the end user, can only be of type `jwt` right now. Must be an object
      that looks like this:
      `typescript                {                  type: 'jwt',                  token: '<your-end-user-jwt-token>'                }                `
