...
When using the fully client-side approach, you will need to authenticate your users against Versori APIs using
[Signing-Key](/v2/embedded/security) tokens.
...
issue a JWT signed with a [Signing-Key](/v2/embedded/security). This token can then be used to authenticate your user
...
- Your organisation ID, obtaining this is covered in the [concepts](/v2/embedded/concepts) section.
...
[comment]: <> (TODO: The below section needs work because we also need API's to support this. It follows in much the
same way as hubs in the old world. But we need an equivalent of ListEndUserIntegrations that lists environments from a
users perspective.)
...
<Accordion title="Activation Diagram">
    ...
    <Frame>
        <img
            src="/legacy/embedded/developer/activation-activity-diagram.svg"
            alt="activity diagram for activating an integration"
        />
    </Frame>
