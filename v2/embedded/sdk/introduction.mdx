...
When using the fully client-side approach, you will need to authenticate your users against Versori APIs using [Signing-Key](/v2/embedded/security) tokens.
...
{/**
TODO: The below section needs work because we also need API's to support this. It follows in much the same way as hubs
 in the old world. But we need an equivalent of ListEndUserIntegrations that lists environments from a users perspective.
