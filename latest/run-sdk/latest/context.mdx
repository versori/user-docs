...
documentation for the [`Context`](https://jsr.io/@versori/run@0.8.1/doc/~/Context) object for more details.
...
const myTask = fn('my-task', async (ctx) => {
    ...
    // Batch operations act on many keys in a single request. `setMany` upserts and reports per-entry
    // successes/failures; `insertMany` is atomic and insert-only (rejects if a key already exists);
    // `deleteMany` removes many keys (deleting a missing key is not an error).
    const { failures } = await kv.setMany([
        { key: ['users', '1'], value: { name: 'Ada' } },
        { key: ['users', '2'], value: { name: 'Alan' } },
    ]);
    if (failures.length) {
        ctx.log.error('Some KV writes failed', { failures });
    }

    await kv.insertMany([{ key: ['users', '3'], value: { name: 'Grace' } }]);
    await kv.deleteMany([['users', '1'], ['users', '2']]);
    ...
    //
    // See the [`CredentialProvider`](https://jsr.io/@versori/run@0.8.1/doc/~/CredentialsProvider) SDK documentation
