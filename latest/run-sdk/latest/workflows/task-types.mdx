...
The `parallel()` method is similar to `serial()`, but runs the task in parallel, which means the task will be run 
against each element of the array at the same time:
