...
When you run `versori projects sync`, a `.versori` file is created in the target directory containing the project ID and context name. This allows subsequent commands like `save` and `deploy` to infer the project and context automatically when run from that directory.
