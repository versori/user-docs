...
Imagine you have a list of users, and you want to remove any users that aren't active for use further along your workflows. You can do this by using the filter action and setting the condition to `user.active` + `Equal to` + `true`.

<Note>
  Remember that the Filter passes forward values that pass the defined rules and
  conditions, rather than filtering out those values.
</Note>
