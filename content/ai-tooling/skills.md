...
|---|---|---|---|
| `name` | string | Yes | A short, unique name for the skill, shown in the library and the integration's skill list. |
| `description` | string | Yes | One line describing what the skill covers and when the AI should apply it. The planning agent uses this to decide whether the skill is relevant. |
| `category` | string | No | An optional grouping label (for example `ELR` or `AvaTax`). Skills with the same category can be attached together, and untagged skills apply regardless of category. You can also set the category at upload time, which writes it into the frontmatter for you. |
...
{{% steps %}}
1. **Attach skills to an integration**
...
- [Versori Skill](/ai-tooling/versori-skill/) — the built-in `coding-versori-sdk` skill that ships with the CLI
- [Claude Code Plugin](/ai-tooling/overview/) — using Versori skills inside AI coding tools
- [Plan agent](/how-it-works/plan/) — how Versori plans integrations
