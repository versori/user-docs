...
const myTask = fn('my-task', async (ctx) => {
    ...
    // The `createIssue` method creates an issue in the owner's organisation — visible in the Issues UI
    // regardless of notification setup. Link an email channel (optional) for proactive alerts.
    await createIssue({
