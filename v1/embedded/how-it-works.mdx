...
A Hub is a collection of integrations that are available to your end users. Typically most users will only require a
single hub, however using multiple is possible. Integrations, End Users and Activations are all associated with a Hub.
...
Boards are different to regular boards created through [Versori Integrate](/v1/integrate) in that the Connections need
not be defined before deploying the Integration. Instead, these Boards use Connection Templates, which define how an End
...
<Note>It is recommended to use the same user ID which you assign to your users, however this is not mandatory.</Note>

When an End User is created, they must provide their connection details for the Primary Connector, and optionally any
Secondary Connectors which are available to them. End Users may be created ahead of time via API, or created immediately
before the first activation of an Integration.
...
referencing an existing [Connection](/v1/concepts/connections) which the End User has already created, or by creating a
new one.
